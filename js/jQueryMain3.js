...
$("#btnFetch").click(function(e){
	...
	var request = $.ajax({
		...
		type: 'GET',
		dataType: 'json',
		//contentType: "application/json; charset=utf-8", //use with POST
		cache: false,
		async: true,
		timeout: 300000,
		
		error: function (jqXHR, textStatus, errorThrow){
			console.log("Bad connection or data, timeout, invalid url error! " + jqXHR.status + " " + textStatus + " " + errorThrow);
		},
		success: function (response, textStatus, jqXHR){
			console.log(textStatus);
			console.log(jqXHR.responseJSON[0]); //jqXHR contain the same as the response
			renderHTMLwithjQuery(response);
			//if you need to convert your data to JSON
			//JSON.parse(response);
		}
